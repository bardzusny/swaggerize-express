 ...
 * @returns {function}
 */
var makeValidator = function(validator, options) {
    ...
    function validateInput(req, res, next) {
        ...
        validate(value, function(error, newvalue) {
            var validateErrorHandler = options.validateErrorHandler || defaultValidateErrorHandler;
            if(error) {
                validateErrorHandler(error, req, res, next);
            }
            else {
...
function defaultValidateErrorHandler(error, req, res, next) {
    res.send(400);
    next(error);
}
 ...
 * @param routeSpec
 */
var makeExpressRoute = function(router, options, route) {
    var path = route.path.replace(/{([^}]+)}/g, ':$1');
    var args = [options.mountpath + utils.prefix(path, '/')];
    var before = [];
    ...
    for (var i = 0; i < route.validators.length; ++i) {
        validators.push(makeValidator(route.validators[i], options));
    }
...
function restifyroutes(router, options) {
    ...
    options.mountpath = options.mountpath || mountpath;
    ...
    routes.forEach(function(route) {
        makeExpressRoute(router, options, route);
    });
}
